...
export default function EditPatientPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    else { toast.success('Paciente actualizado'); router.push(`/patients/${id}`) }
    setSaving(false)
  }
  ...
  return (
    <div className="p-8 animate-fade-in max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href={`/patients/${id}`} className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors">
          <ArrowLeft size={18} className="text-slate-500" />
        </Link>
        <div>
          <h1 className="text-2xl text-slate-800">Editar paciente</h1>
          <p className="text-sm text-slate-500">Modificá los datos del paciente</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <textarea value={form.admin_notes} onChange={e => set('admin_notes', e.target.value)}
            placeholder="Obra social, forma de pago, derivación, etc."
            rows={3} />
        </div>
